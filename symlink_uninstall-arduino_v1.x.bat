...
:ADMIN
	...
	FOR /f "usebackq delims=|" %%f in (`dir /a:d /b "%~dp0src\"`) DO (
		::cmd /c rmdir "%arduino_filedrive%%arduino_filepath%libraries\%%f"
		rmdir "%arduino_filedrive%%arduino_filepath%libraries\%%f"
	)
...
:NOADMIN
	...
	FOR /f "usebackq delims=|" %%f in (`dir /a:d /b "%~dp0src\"`) DO (
		::cmd /c rmdir "%arduino_filedrive%%arduino_filepath%libraries\%%f"
		ECHO rmdir "%arduino_filedrive%%arduino_filepath%libraries\%%f"
	)
	ECHO.
